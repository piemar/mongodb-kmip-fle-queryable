...
vault server -config=csfle_vault_kmip/vault/config.hcl &
sleep 5
...
jq -r .data.certificate < credential.json > csfle_vault_kmip/certs/vv-ca.pem
jq -r .data.private_key < credential.json > csfle_vault_kmip/certs/vv-key.pem    
cat csfle_vault_kmip/certs/vv-ca.pem csfle_vault_kmip/certs/vv-key.pem > csfle_vault_kmip/certs/vv-client.pem
vault read demo/kmip/scope/mongodb/role/FLE/credential/lookup \serial_number=$serial_number -format=json > ca_chain.json
jq -r .data.certificate < ca_chain.json > csfle_vault_kmip/certs/vv-ca.pem    
rm -rf *.json
