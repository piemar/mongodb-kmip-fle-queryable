...
jq -r .data.certificate < credential.json > kmip-with-hashicorp-key-vault/certs/vv-ca.pem
jq -r .data.private_key < credential.json > kmip-with-hashicorp-key-vault/certs/vv-key.pem    
cat kmip-with-hashicorp-key-vault/certs/vv-ca.pem kmip-with-hashicorp-key-vault/certs/vv-key.pem > kmip-with-hashicorp-key-vault/certs/vv-client.pem
vault read demo/kmip/scope/mongodb/role/FLE/credential/lookup \serial_number=$serial_number -format=json > ca_chain.json
jq -r .data.certificate < ca_chain.json > kmip-with-hashicorp-key-vault/certs/vv-ca.pem    
rm -rf *.json
